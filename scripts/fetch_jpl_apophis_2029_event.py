...
def main() -> int:
    ...
    refine5_params.update({"TLIST": tlist(float(coarse_min["jd_tdb"]), 3*3600, 300), "TLIST_TYPE": "'JD'"})
    ...
    refine30_params.update({"TLIST": tlist(float(refine5_min["jd_tdb"]), 10*60, 30), "TLIST_TYPE": "'JD'"})
    ...
    write(SUMMARY_JSON, summary_text)
    ...
    print(json.dumps({"final_jd_tdb": final_min["jd_tdb"], "final_range_au": final_min["range_au"], "summary_sha256": summary_sha}, indent=2))
    return 0
